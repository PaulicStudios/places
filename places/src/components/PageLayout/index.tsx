...
export const Page = (props: { children: ReactNode; className?: string }) => {
  return (
    <div className={twMerge(clsx('flex h-screen flex-col relative', props.className))}>
      {props.children}
    </div>
  );
};
...
const Header = (props: { children: ReactNode; className?: string }) => {
  return (
    <header
      className={twMerge(
        'bg-white flex flex-col justify-center px-6 pt-6 pb-3 z-50 fixed top-0 left-0 right-0 border-b border-gray-100 fixed-header',
...
const Main = (props: { children: ReactNode; className?: string }) => {
  return (
    <main
      className={twMerge(
        clsx('grow overflow-y-auto p-6 pt-20', props.className),
      )}
    >
      {props.children}
    </main>
